...
- **CLI Terminal Interactif (Style Mimocode)** : Une expérience ultra-moderne basée sur **Ink** (React pour terminal). Inclut l'autocomplétion intelligente, l'historique persistant, le rendu Markdown riche, et un monitoring système en temps réel (CPU/RAM).
...
### 1. Prérequis
- **Node.js** (v18+)
- **Ollama** (recommandé) ou tout autre runtime compatible OpenAI/Mimocode local.
...
#### Raccourcis Clavier (Interface Ink)
- `↑` / `↓` : Naviguer dans l'historique des commandes.
- `Tab` : Autocomplétion des `@agents` et `/commands`.
- `Ctrl + C` : Interrompre l'opération en cours ou quitter.
- `Ctrl + L` : Effacer l'écran.
- `\` (en fin de ligne) : Passer en mode saisie multi-ligne.
...
**Nouveau :** Lors d'une modification de fichier par l'IA, Mimocode affiche désormais un **diff coloré** (style Mimocode CLI) vous permettant de valider précisément les changements avant qu'ils ne soient appliqués.
