  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      if (await fs.pathExists(skillMdPath)) {
        ...
        // Simple parsing of SKILL.md (assuming frontmatter or similar structure)
        // For now, let's just use the filename as name and content as prompt
        skills.push({
          name: file,
          description: `Custom skill from ${file}`,
          prompt: content
        });
      }
    } else if (file.endsWith('.json')) {
      skills.push(await fs.readJson(fullPath));
    }
  }
