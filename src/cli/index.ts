...
// Configurer la notification de mise à jour
try {
  ...
  const notifier = updateNotifier({
    pkg,
    updateCheckInterval: 1000 * 60 * 60 * 24, // 1 jour
    shouldNotifyInNpmScript: true,
  });

  notifier.notify({
    isGlobal: true,
    message: 'Update available: {currentVersion} → {latestVersion}\nRun `npm update -g mimocode` to update',
  });
} catch (e) {
  // Silent error for update notification
}
...
// ============================================
// MIMOCODE INTERFACE STATE & COMPONENTS
// ============================================
...
function getSuggestions(input: string, agents: Agent[], skills: any[]): Suggestion[] {
  ...
  if (input.startsWith('@')) {
    ...
    if (lastWord.includes('/') || lastWord.includes('.')) {
      try {
        const dir = lastWord.includes('/') ? path.dirname(lastWord) : '.';
        const base = lastWord.includes('/') ? path.basename(lastWord) : lastWord;
        const fullDir = path.resolve(process.cwd(), dir);
        if (fs.existsSync(fullDir)) {
          const files = fs.readdirSync(fullDir);
          files.forEach(f => {
            if (f.toLowerCase().startsWith(base.toLowerCase())) {
              suggestions.push({ type: 'command', value: path.join(dir, f), description: 'File path' });
            }
          });
        }
      } catch (e) {}
...
function showDiff(filePath: string, oldContent: string, newContent: string) {
  ...
  // Simple line-by-line diff (for demo purposes, a real diff lib would be better)
  let i = 0, j = 0;
  while (i < oldLines.length || j < newLines.length) {
    if (i < oldLines.length && j < newLines.length && oldLines[i] === newLines[j]) {
      // Unchanged (skip or show dim)
...
async function handleSlashCommand(input: string, config: Config, agents: Agent[], skills: any[]) {
  ...
  switch (command) {
    ...
    case 'search':
      ...
      // Check if it's a document search (if a path is provided or if we want to search in docs)
...
process.on('unhandledRejection', (reason, promise) => {
  console.error(chalk.red('\nCRITICAL: Unhandled Rejection at:'), promise, chalk.red('reason:'), reason);
});

process.on('uncaughtException', (err) => {
  console.error(chalk.red('\nCRITICAL: Uncaught Exception:'), err);
  // Give some time for the log to be seen
...
program
  .name('mimocode')
  .description('Mimocode - Autonomous AI Engineer')
  .version('0.36.4');
