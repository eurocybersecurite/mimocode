...
const getSafePath = (filePath: string) => {
...
export const mcpTools: MCPTool[] = [
  {
    name: 'invoke_skill',
    description: 'Execute a high-level skill procedure. Pass the skill name and the prompt input.',
    execute: async (args) => {
      const { skillName, prompt } = args;
      if (!skillName || !prompt) {
        throw new Error('skillName and prompt are required for invoke_skill');
      }
      ...
      return await runSkill(config, skillName, finalPrompt);
    },
  },
  {
    name: 'activate_skill',
    description: 'Activate a specialized skill by name. This will load the skill context from the filesystem and prepare it for execution.',
    execute: async (args) => {
      const { skillName } = args;
      if (!skillName) throw new Error('skillName is required for activate_skill');
      ...
      if (!(await fs.pathExists(mdPath))) {
        return `Skill context ${skillName}.md not found. Proceeding with standard skill definition.`;
      }
      
      const context = await fs.readFile(mdPath, 'utf-8');
      return `Skill ${skillName} activated. Context loaded:\n${context}`;
    },
  },
  {
    name: 'read_file',
    ...
    execute: async (args) => {
      ...
      const content = await fs.readFile(getSafePath(filePath), 'utf-8');
      return content;
    },
  },
  {
    ...
    execute: async (args) => {
      ...
      if (verification.success) {
        return `Successfully wrote to ${filePath}. ${verification.message}`;
      } else {
